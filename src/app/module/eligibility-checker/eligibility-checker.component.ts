...
export class EligibilityCheckerComponent implements OnInit {
  ...
  ngOnInit(): void {
    // Update meta tags for loan eligibility calculator
    this.updateMetaTags();
  }
  ...
  downloadPDF(): void {
    ...
    // Add loan details
    doc.setFontSize(12);
    doc.text(`Loan Type: ${this.selectedLoanType?.viewValue || 'N/A'}`, 20, 40);
    doc.text(`Monthly Salary: ₹${this.monthlySalary?.toLocaleString() || 'N/A'}`, 20, 50);
    doc.text(`Loan Amount: ₹${this.loanAmount?.toLocaleString() || 'N/A'}`, 20, 60);
    doc.text(`Loan Tenure: ${this.tenureMonths} months`, 20, 70);
    doc.text(`Existing EMIs: ₹${this.expenses?.toLocaleString() || '0'}`, 20, 80);
    
    // Add results
    doc.setFontSize(14);
    doc.text('Eligibility Results:', 20, 100);
    doc.setFontSize(12);
    doc.text(`Status: ${this.result.eligible ? 'APPROVED' : 'NOT APPROVED'}`, 20, 110);
    doc.text(`Monthly EMI: ₹${this.result.emi.toLocaleString()}`, 20, 120);
    doc.text(`EMI to Income Ratio: ${this.result.ratio.toFixed(2)}%`, 20, 130);
    doc.text(`Explanation: ${this.explanation}`, 20, 140);
  ...
  downloadExcel(): void {
    ...
    const worksheet = XLSX.utils.json_to_sheet([
      {
        'Loan Type': this.selectedLoanType?.viewValue || 'N/A',
        'Monthly Salary': this.monthlySalary,
        'Loan Amount': this.loanAmount,
        'Loan Tenure (months)': this.tenureMonths,
